...
from popups import ModbusPopup, ScanPopup, MedidoresPopup, MedidoresMotorPopup, GraficoMotorPopup, MotoresPopup, DadosPopup, OutrosPopup, GraficoArSaiPopup, GraficoTit01Popup, GraficoTit02Popup, GraficoPit01Popup, GraficoPit02Popup, GraficoPit03Popup, HistGraphPopup
...
class MainWidget(BoxLayout):
        ...
        def updateGUI(self):
                ...
                popups = {
                        '_medidoresPopup': [('corrente_r', ' A'), ('corrente_s', ' A'), ('corrente_t', ' A'),
                        ...
                        , ('frequencia_compressor', 'Hz')],
                        '_medidoresMotorPopup':[ ('frequencia_motor', 'Hz'), ('torque_motor_axial', 'N.m')]
                ...
                self.ids.lb_motor.size = (self._meas['values']['vel_motor']/3800*60,15)
                self.ids.lb_pitum.size = (self._meas['values']['pressao_canAzul']/15*60,15)
                self.ids.lb_pitdois.size = (self._meas['values']['pressao_canVerm']/40*60,15)
                self.ids.lb_pittres.size = (self._meas['values']['pressao_arEnt']/15*60,15)
                self.ids.lb_tempsai.size = (self._meas['values']['temperatura_arSaida']/40*60,15)
                self.ids.lb_velsai.size = (self._meas['values']['velocidade_arSaida']/15*60,15)
        ...
        def LigaMotor(self):
                ...
                if not self._motorLigado:
                        ...
                        if tipo_partida == 1:
                                self._modbusClient.write_single_register(1316, 1) #soft-start

                        elif tipo_partida == 2:
                                self._modbusClient.write_single_register(1312, 1) #inversor
                        else:
                                self._modbusClient.write_single_register(1319, 1) #direta
                else:
                        self._motorLigado = False
                        if tipo_partida == 1:
                                self._modbusClient.write_single_register(1316, 0) #soft-start
                        elif tipo_partida == 2:
                                self._modbusClient.write_single_register(1312, 0) #inversor
                        else:
                                self._modbusClient.write_single_register(1319, 0) #direta
